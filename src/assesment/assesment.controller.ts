...
export class AssesmentController implements CrudController<Assessment> {
    ...
    try {
      if (dto.assessmentType != 'MAC') {
        ...
        for await (const a of par2) {
          ...
          if (a.value) {
            // added for make null for default values
            a.institution = null;
          }
        ...
        //   ).assessmentObjective,
        // );
        const objectives = await queryRunner.manager.save(
          AssessmentObjective,
          dto.assessmentObjective,
        );
        // let objectives = await this.assessmentObjectiveRepo.save(
        //   (
        //     await dto
  ...
  async testTransaction(@Request() request): Promise<any> {
    ...
    try {
      ...
      const paeameter = await queryRunner.manager.save(Parameter, para);
      await queryRunner.commitTransaction();
      console.log('worktran11');
